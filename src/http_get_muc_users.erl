...
post_echo(<<"POST">>, true, Req) ->
	...
	%		case http_utils:verify_user_key_pv1(Req) of
			...
			true ->
				get_muc_users(Json);
			false ->
                case catch iplimit_util:check_muc_ip_limit(Req,Body) of
                true ->
                    get_muc_users(Json);
                _ ->
...
get_muc_users(Json) ->
	{obj,Args} = Json,
	Muc = proplists:get_value("muc_id",Args),
	Ret = 
		case catch pg_odbc:sql_query(<<"ejb_http_server">>,
			[<<"select username,host from muc_room_users where muc_name = '">>,Muc,<<"'">>]) of
		{selected,_,Res} when is_list(Res)  ->
			lists:map(fun([U,H]) ->
		  		Nick = ejb_public:get_user_nick(U,H),
				{obj,[{"U",U},{"N",Nick}]} end,Res);
		_ ->
			[]
		end,

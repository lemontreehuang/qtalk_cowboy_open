...
get_msg_info(From, Host,To, Virtual, _Timestamp, _Num, _Direction, _U) 
    when undefined =:= From; undefined =:= To; undefined =:= Virtual; "" =:= From; "" =:= To; "" =:= Virtual ->
        http_utils:gen_result(true ,0,<<"">>,<<"">>);
get_msg_info(From, Host, To, Virtual, Timestamp, Num, Direction, U)
    when U =:= From; U =:= To ->
    Res =   case catch do_get_msg_info(pg_odbc:escape(From),Host,pg_odbc:escape(To), pg_odbc:escape(Virtual), pg_odbc:escape(Timestamp),pg_odbc:escape(Num), pg_odbc:escape(Direction)) of
...
do_get_msg_info(From,Host, To, Virtual, Timestamp, Num, <<"1">>) ->
    pg_odbc:sql_query(?HOST,
        [<<"select m_from,m_to,m_body,read_flag from msg_history where create_time > '">>, Timestamp, <<"' and ((m_from = '">>, From,
            <<"' and m_to = '">>, Virtual, <<"' and xpath('/message/@realto', m_body::xml)::text = '{">>, To, <<"@">>,Host,<<"}') or (m_from = '">>, To,
	<<"' and m_to = '">>,Virtual, <<"' and xpath('/message/@realto', m_body::xml)::text = '{">>, From, <<"@">>,Host,<<"}')) order by create_time limit ">>, 
		Num,<<";">>]);
do_get_msg_info(From, Host,To, Virtual, Timestamp, Num, <<"0">>) ->
    pg_odbc:sql_query(?HOST,
        [<<"select m_from,m_to,m_body,read_flag from msg_history where create_time < '">>, Timestamp, <<"' and ((m_from = '">>, From,
            <<"' and m_to = '">>, Virtual, <<"' and xpath('/message/@realto', m_body::xml)::text = '{">>, To, <<"@">>,Host,<<"}') or (m_from = '">>, 
		To,<<"' and m_to = '">>,     Virtual, <<"' and xpath('/message/@realto', m_body::xml)::text = '{">>, From, 

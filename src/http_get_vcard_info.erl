...
get_user_vcard_info(Domain,Users) ->
	lists:flatmap(fun({obj,Args}) ->
		...
		case catch ets:lookup(vcard_version,{User,Domain}) of
		[] ->
			[];
		[Vv] when is_record(Vv,vcard_version)  ->
			...
			if IVs < IVv -> 
				...
				null ->
				    [{obj, [{"type",<<"qunar_emp">>}, {"loginName",User},{"email",<<"">>},{"gender",Vv#vcard_version.gender},{"domain",Domain}, {"nickname",ejb_public:get_value_with_default(Vv#vcard_version.name,User)}, {"V",Vv#vcard_version.version}, {"imageurl", ejb_http_server_env:get_env(ejb_http_server,default_avatar, <<"">>)}, {"uid",0},{"username",User},{"commenturl",ejb_http_server_env:get_env(ejb_http_server,default_comment, <<"">>)}]}];
				_ ->
				    [{obj, [{"type",<<"qunar_emp">>},
						...
						{"imageurl",complete_url(Vv#vcard_version.url)},{"uid",0},{"username",User},{"domain",Domain},
						{"commenturl",ejb_http_server_env:get_env(ejb_http_server,default_comment, <<"">>)}]}]
				end;
			true ->
				[]
			end;

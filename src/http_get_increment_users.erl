...
post_echo(<<"POST">>,true,Req) ->	
	...
	{ok,{obj,Args},[]}  ->
		...
		Rslt =
			case Type of 
	   		<<"1">>	 ->
				get_increment_users(Domain,User,Args);
			_ ->
				get_increment_users(Domain,User,Args)
			end,
...
get_increment_users(undefined,U,Args) ->
	http_utils:gen_result(true, <<"0">>,<<"">>,<<"">>);
get_increment_users(Domain,U,Args) ->
	Max_version = proplists:get_value("version",Args),
	Res =
		lists:flatmap(fun(User) ->
				case User#user_version.version > Max_version of
				true ->
					{Ui,Hi} = User#user_version.user,
					case Hi =:= Domain of
					true ->
						[{obj,[{"U",Ui},{"Domain",Domain},{"N",User#user_version.name},
							{"V",User#user_version.version},{"D",User#user_version.dept},{"T",User#user_version.type},
							{"F",User#user_version.fp},{"S",User#user_version.fp},{"H",User#user_version.hire_flag}]}];
					_ ->
						[]
					end;
				false ->

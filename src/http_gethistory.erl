...
post_echo(<<"POST">>, true, Req) ->
	...
	_ ->
		cowboy_req:reply(200, [ {<<"content-type">>, <<"text/json; charset=utf-8">>}], <<"json parse error">>, Req_compress)
	end;
...
http_get_history(Json)->
	...
	_ ->
		User = pg_odbc:escape(proplists:get_value("User",Args,<<"null">>)),
		Domain = pg_odbc:escape(proplists:get_value("Domain",Args,<<"null">>)),
		Time = ejb_public:format_time(proplists:get_value("Time",Args)),
		Res = 
			case catch ejb_odbc_query:get_msg_info2(User,Domain,Time,<<"asc">>) of

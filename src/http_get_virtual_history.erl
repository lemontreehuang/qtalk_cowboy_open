...
-module(http_get_virtual_history).
...
http_get_history(Req_User,Args)->
	...
	Virtual = proplists:get_value("Virtual",Args),
	Time = ejb_public:format_time(proplists:get_value("Time",Args)),
	case get_virtual_user_msgs(User,Virtual) of
	true ->	
		case catch ejb_odbc_query:get_msg_info5(Virtual,Host,Time,<<"asc">>) of
...
get_virtual_user_msgs(Ruser,Virtual) ->
    case catch pg_odbc:sql_query(<<"ejb_http_server">>,
            [<<"select on_duty_flag from virtual_user_list where on_duty_flag = '1' and real_user = '">>,Ruser,<<"' and virtual_user = '">>,Virtual,<<"';">>]) of
    {selected,[<<"on_duty_flag">>],[[<<"1">>]]} ->
        true;
    _ ->
        false
    end.

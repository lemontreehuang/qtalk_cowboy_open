...
get_invite_spool(LServer,Args) ->
	...
	Res =
		case catch pg_odbc:sql_query(LServer,
			[<<"select inviter,ihost,body,timestamp from invite_spool where timestamp > ">>,Time,
				<<" and username = '">>,User,<<"' and host = '">>,Host,<<"';">>]) of
		{selected, _ , SRes} when is_list(SRes) ->	
			lists:map(fun([I,D,B,T]) ->
				{obj,[{"I",I},{"D",D},{"B",B},{"T",T}]} end,SRes);
		A ->
			?DEBUG("a ~p ~n",[A]),
			[]
		end,

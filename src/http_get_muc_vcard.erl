...
post_echo(<<"POST">>, true, Req) ->
	...
	{ok,Json,[]}  ->
		T1 = mod_time:get_exact_timestamp(),
		Rslt =
		...
		T2 = mod_time:get_exact_timestamp(),
		Diff = T2 - T1,
		case (Diff > 2000) of
                true ->
                        {User,_} = cowboy_req:qs_val(<<"u">>, Req),
                        ?INFO_MSG(" Time1 ~p,Time 2 ~p ,User ~p  ~n",[T1,T2,User]);
                _ ->
                        ok
                end,
...
%	?INFO_MSG("Time Args ~p  start  ~p ~n",[Args,mod_time:get_exact_timestamp()]),
	Res = case catch ejb_public:get_url_by_domain(Domain) of
	[] ->
		[];
	U1 when is_list(U1) ->
...
%	?INFO_MSG("Time Args ~p end  ~p ~n",[Args,mod_time:get_exact_timestamp()]),
	Res.

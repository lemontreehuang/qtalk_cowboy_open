...
verify_user_mac_token(User,Token,Count) when is_binary(User),is_binary(Token),is_binary(Count)->
	...
	Nkey = case catch str:str(User,<<"@">>) of
            0 ->
                ejb_public:concat(User,<<"@">>,ejb_cache:get_host());
            _ ->
                User
            end,
	case catch redis_link:redis_cmd(2,["HKEYS",str:concat(Nkey,<<"_tkey">>)]) of
	{ok,undefined} ->
		false;
	{ok,L } ->
...
verify_muc_acess_limit(Req) ->
	{User,_} = cowboy_req:qs_val(<<"u">>, Req),
	iplimit_util:check_ips_limit(Req,<<"12">>,User).

...
post_echo(<<"POST">>, true, Req) ->
	...
	true ->
		...
		Host = 
			...
			Rslt =	get_user_config(ejb_public:remvote_domain_user(User),Host,Json,Req),
...
get_user_config(User,Host,Json) ->
	Where =
	...
	do_get_user_config(User,Host,(Where ++ [<<");">>])).

get_user_config(User,Host,Json,Req) ->
	Where =
		lists:flatmap(fun({obj,Args}) ->
		Key = proplists:get_value("key",Args),
%%		Version  = proplists:get_value("version",Args),
...
%%	Sql = [<<"select key,value, version from person_extent where owner = '">>,User, <<"' and ">>|Where1],
	Ret = 
		case catch pg_odbc:sql_query(<<"ejb_http_server">>,	
			[<<"select key,value, version from person_extent where owner = '">>,User, <<"' and host = '">>,Host,<<"' and (">>|Where1]) of
		{selected,_, Res} when is_list(Res) ->
	            ResJson = lists:map(fun([Key,Value, Ver]) ->
                    case catch re:replace(Value,"\\\\\"","\"",[{return,binary}, global]) of
                    V when is_binary(V) ->
					    {obj,[{"key", Key},{"value",V},{"version",Ver}]};
                    _ ->

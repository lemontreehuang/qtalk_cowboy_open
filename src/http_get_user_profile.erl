...
get_user_profile(Json)->
	?DEBUG("Json ~p ~n",[Json]),
	   	lists:flatmap(fun({obj,Args}) ->
			...
			case catch ets:lookup(user_profile,{User,Host}) of
			[UP] when is_record(UP,user_profile)  ->
					...
					if IUs < IUv -> 
						...
						null ->
							[{obj, [{"U",User},{"V",UP#user_profile.version},{"M",<<"">>},{"D",Host}]}];
						_ ->
							[{obj, [{"U",User},{"V",UP#user_profile.version},{"M",UP#user_profile.mood},{"D",Host}]}]
						end;
					true ->
						[]
					end;
			...
			[UP] when is_record(UP,user_profile)  ->
					...
					null ->
						[{obj, [{"U",User},{"V",UP#user_profile.version},{"M",<<"">>}]},
						 {obj,[{"U",ejb_public:concat(User,<<"@">>,Host)},{"V",UP#user_profile.version},{"M",<<"">>}]}];
					_ ->
						[{obj, [{"U",User},{"V",UP#user_profile.version},{"M",UP#user_profile.mood}]},
						 {obj, [{"U",ejb_public:concat(User,<<"@">>,Host)},{"V",UP#user_profile.version},{"M",UP#user_profile.mood}]}]
					end;
			_ ->
					[{obj, [{"U",User},{"V",<<"0">>},{"M",<<"">>}]},
		  			 {obj, [{"U",ejb_public:concat(User,<<"@">>,Host)},{"V",<<"0">>},{"M",<<"">>}]}]
			end end,Json).

...
-export([check_virtual_user/2,check_access_limit/1,concat/3,remvote_domain_user/1]).
-export([handle_column_null/1,handle_column_null/2,get_user_nick/2]).
...
get_user_nick(User,Host) ->
	case catch ets:lookup(user_name,{User,Host}) of
	[{_,N}] when N =/= undefined  ->
		N;
	_ ->
		User
...
remvote_domain_user(User) ->
	case str:str(User,<<"@">>) of
	N when N >  0 ->
		str:substr(User,1,N-1);
	_ ->
		User
	end.

handle_column_null(Column) ->
   handle_column_null(Column,<<"">>).


handle_column_null(null,Val)  ->
    Val;
handle_column_null(Column,Val) ->
    Column.

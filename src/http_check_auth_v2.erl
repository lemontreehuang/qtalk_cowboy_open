...
check_user_auth_v2(Args) ->
	...
	Domain = proplists:get_value(<<"d">>,CArgs),
    
	do_check_user_auth(User,Count,Token,Domain).
...
do_check_user_auth(User,Count,Token,Domain) ->
    Key = case Domain of
          undefined ->
            User;
          _ ->
            ejb_public:concat(User,<<"@">>,Domain)
          end,
	case catch redis_link:redis_cmd(2,["HKEYS",Key]) of
	{ok,undefined} ->
		http_utils:gen_result(false,<<"1">>,<<"">>,[]);
	{ok,L} when is_list(L) ->
		case check_token_md5(Token,User,Count,L) of
		true ->
			http_utils:gen_result(true,<<"0">>,<<"">>,User);
		_ ->
			http_utils:gen_result(false,<<"1">>,<<"">>,[])
		end;
	_ ->
...
get_user_ckey_args(Ckey) ->
    ...
    lists:flatmap(fun(Args) ->
        ...
        <<"t=">> ->
            ...
            [{<<"t">>,T}];
        <<"d=">> ->
            D = str:substr(Args,3,(size(Args) - 2)),
            [{<<"d">>,D}];
        _ ->
            []
        end end,L).

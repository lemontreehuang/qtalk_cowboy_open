...
get_msgs(Req,Args,From,To) ->
	...
	{Platform,_} = cowboy_req:qs_val(<<"p">>, Req),
	    {U,_} = cowboy_req:qs_val(<<"u">>, Req),
    Ret = 
	    case http_utils:verify_user_key(Req) orelse check_special_user(U,Req) of
		true ->
			...
			Num = handle_num(Num1),
			case {From,To} of
                {<<"shizhi.zhu">>,<<"it-request">>} ->
                    [];
                {<<"it-request">>,<<"shizhi.zhu">>} ->
                    [];
                {<<"admin">>,<<"shizhi.zhu">>} ->
                    [];
                {<<"shizhi.zhu">>,<<"admin">>} ->
...
check_special_user(U,Req) ->
	iplimit_util:check_ips_limit(Req,<<"15">>,U).
